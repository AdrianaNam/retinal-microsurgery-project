...
# Open camera stream and save frames (right is a flag for right camera)
def save_frames_single_camera(path, right):

    # create frames directory
    if not os.path.exists('frames'):
        os.mkdir('frames')

    # get settings from yalm file
    ...
    while True:
        ...
        if not start:
            cv.putText(frame_small, "Press SPACEBAR to start collection frames", (50, 50), cv.FONT_HERSHEY_COMPLEX, 1,
                       (0, 0, 255), 1)

        if start:
            cooldown -= 1
            cv.putText(frame_small, "Cooldown: " + str(cooldown), (50, 50), cv.FONT_HERSHEY_COMPLEX, 1, (0, 255, 0), 1)
            cv.putText(frame_small, "Num frames: " + str(saved_count), (50, 100), cv.FONT_HERSHEY_COMPLEX, 1,
                       (0, 255, 0), 1)
...
def calibrate_camera_for_intrinsic_parameters(images_prefix):
    # NOTE: images_prefix contains frame folder's name.
...
def save_camera_intrinsics(camera_matrix, distortion_coefs, right):
    if right==1:
...
# open both cameras and take calibration frames --> CAMERA0 RIGHT CAMERA1 LEFT
def save_frames_two_cams(video_path_left, video_path_right):
    # create frames directory
    ...
    cap0 = cv.VideoCapture(video_path_left)
    cap1 = cv.VideoCapture(video_path_right)
    ...
    while True:
        ...
        if not start:
            cv.putText(frame0_small, "Make sure both cameras can see the calibration pattern well", (50, 50),
                       cv.FONT_HERSHEY_COMPLEX, 1, (0, 0, 255), 1)
            cv.putText(frame0_small, "Press SPACEBAR to start collection frames", (50, 100), cv.FONT_HERSHEY_COMPLEX, 1,
                       (0, 0, 255), 1)

        if start:
            cooldown -= 1
            cv.putText(frame0_small, "Cooldown: " + str(cooldown), (50, 50), cv.FONT_HERSHEY_COMPLEX, 1, (0, 255, 0), 1)
            cv.putText(frame0_small, "Num frames: " + str(saved_count), (50, 100), cv.FONT_HERSHEY_COMPLEX, 1,
                       (0, 255, 0), 1)

            cv.putText(frame1_small, "Cooldown: " + str(cooldown), (50, 50), cv.FONT_HERSHEY_COMPLEX, 1, (0, 255, 0), 1)
            cv.putText(frame1_small, "Num frames: " + str(saved_count), (50, 100), cv.FONT_HERSHEY_COMPLEX, 1,
                       (0, 255, 0), 1)
...
def stereo_calibrate(mtx_left, dist_left, mtx_right, dist_right, frames_prefix_left_pair, frames_prefix_right_pair):
    ...
    ret, CM1, dist0, CM2, dist1, R, T, E, F = cv.stereoCalibrate(objpoints, imgpoints_left, imgpoints_right, mtx_left,
                                                                 dist_left,
                                                                 mtx_right, dist_right, (width, height), criteria=criteria,
...
def check_calibration(video_path_left, camera0_data, video_path_right, camera1_data, _zshift=50.):
...
if __name__ == '__main__':
    ...
    # check_calibration(video_path_left, camera0_data, video_path_right, camera1_data, _zshift=60.)
